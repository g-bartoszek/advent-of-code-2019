...
fn move_drone(input: &mut std::sync::mpsc::SyncSender<IntCode>,
          output: &mut std::sync::mpsc::Receiver<IntCode>,
          drone: &mut (i64, i64),
          map: &mut std::collections::HashMap<(i64, i64), char>,
          key: &str) -> i64 {

    let (command, target) = match key {
        "s" => { (2, (drone.0, drone.1 - 1))},
        "a" => { (3, (drone.0 - 1, drone.1))},
        "d" => { (4, (drone.0 + 1, drone.1))},
        "w"|_ => { (1, (drone.0, drone.1 + 1))},
    };
    ...
    match result {
        0 => { map.insert(target.clone(), '#'); },
        1 => { *drone = target; map.insert(target.clone(), ' '); },
        2 => { println!("\n\n\n\nOXYGEN!\n\n\n\n\n"); *drone = target; map.insert(target.clone(), 'O'); },
...
fn manual_control(mut input: std::sync::mpsc::SyncSender<IntCode>,
                  mut output: std::sync::mpsc::Receiver<IntCode>,
                  drone: &mut (i64, i64),
                  map: &mut std::collections::HashMap<(i64, i64), char>) {
    ...
    loop {
        ...
        move_drone(&mut input, &mut output, drone, map, buffer.trim());
        buffer.clear();
        print_map(&map, &drone);
    }
}
...
fn main() {
    ...
    loop {
        ...
        if move_drone(&mut input_tx, &mut output_rx, &mut drone, &mut map, direction) == 0 {
            loop {
                direction = match direction {
                    "w" => "d",
                    "d" => "s",
