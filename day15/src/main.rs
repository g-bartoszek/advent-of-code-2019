...
fn main() {
    ...
    let (input_tx, input_rx) = std::sync::mpsc::sync_channel::<interpreter::IntCode>(1);
    ...
    loop {
        ...
        let (command, target) = match buffer.trim() {
            "w" => { (1, (drone.0, drone.1 + 1))},
        ...
        input_tx.send(command).unwrap();

        let result = output_rx.recv().unwrap();

        match result {
            0 => { map.insert(target.clone(), '#'); },
            1 => { drone = target; map.insert(target.clone(), ' '); },
            2 => { map.insert(target.clone(), 'O'); },
